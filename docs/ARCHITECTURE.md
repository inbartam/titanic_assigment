...
This document is the source of truth for module boundaries, the artifact format, the four models
and the Streamlit UI. The code should match it. If the doc turns out to be wrong, fix the doc in
the same commit.
...
stage timing recorded in `MetricsRegistry`. No training data is required at this point. All
models consume the same `(X_num, X_cat)`. Neither `app/` nor `api/` calls a bundle directly; see
...
| `FamilySize` | `SibSp + Parch + 1` | non-monotonic survival (2 to 4 best)                     | no       | yes                             | yes (num) |
...
| `TicketGroupSize` | count of ticket over batch | strong signal on Kaggle                           | yes: batch-dependent, and trivially different for a single row | no | **no** |
...
Order of operations inside `transform`: `engineer` is not called here (the caller does it) →
...
- `F.one_hot(x_cat[:, i], num_classes=card_i)` per categorical col, concatenated with `x_num`
  ...
  loop/loss/optimizer as the other torch models so the comparison is apples-to-apples.
- Config: `{"type": "linear", "lr": 0.01, "weight_decay": 1e-4, "epochs": 200, "batch_size": 64}`.
  No CV, since there is one obvious config.
...
- Default: `h1=64, h2=32, p=0.3, lr=1e-3, weight_decay=1e-3, batch_size=64, max_epochs=300,
  patience=20`. ≈ 3k to 4k parameters.
...
### `attn`: `TitanicAttention` (torch), the cut-first model
...
- `nn.TransformerEncoder(TransformerEncoderLayer(d_model=16, nhead=4, dim_feedforward=64,
  ...
  `LayerNorm` → `Linear(16, 1)`. ≈ 6k to 8k parameters, < 60 s on CPU.
- Single fixed config with no grid (time-boxed). Trained with the same loop, `lr=1e-3`,
...
- It exists because it is the strongest classical reference on small tabular data, and the app
  should let a user see that directly instead of reading about it.
...
3. Decision threshold: slider 0.05 to 0.95 step 0.05, default 0.50, caption: "Default 0.5. Lower
...
- **Compare models**: table of every registered model's `metrics.json` (validation + CI,
  ...
  each torch model (tabs); a short, plain paragraph auto-filled from the numbers ("On the
...
containing the traceback, so the user does not see a raw red traceback. Missing or partial
artifacts → an error telling the user to run `python train.py --model all`. A partial registry
is not an error.
